    ...
    try {
        ...
        function Organizar_Conversa(retornoApiBlip) {
            ...
            try {
                ...
                return `${timestamp}Conversa.txt`

            } catch (error) {

                return "ERRO ENCONTRADO: " + error
            }
        }
...
const enviarArquivoPipedrive = async (arquivo, idPipe, token) => {
    try {
        ...
        const URL = "https://api.pipedrive.com/v1/files?api_token=" + token
...
app.post('/api', async (req, res) => {
    ...
    try {
        ...
        if (!telefone || !id) {
            return res.status(400).json({ status: 'failed', erro: 'Telefone e/ou ID ausente(s)' })
        }

        const telefoneCliente = await obterTelefoneCliente(telefone)
        const caminhoArquivo = await obterConversaCliente(telefoneCliente)
        await enviarArquivoPipedrive(caminhoArquivo, id, tokenPipe)
